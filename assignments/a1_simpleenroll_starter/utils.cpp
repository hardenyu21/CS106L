...
std::string COURSES_OFFERED_CSV_PATH = "/Users/hardenyu/Study/cs106L/assignments/a1_simpleenroll_starter/student_output/courses_offered.csv";

std::string COURSES_NOT_OFFERED_CSV_PATH = "/Users/hardenyu/Study/cs106L/assignments/a1_simpleenroll_starter/student_output/courses_not_offered.csv";
...
void write_courses_offered(std::vector<Course> & vector_of_courses) {
  ...
  std::vector<Course> vector_of_offered_courses;
  if (ofs.is_open()){
    for(int i = 1; i < vector_of_courses.size(); ++i){
      Course course = vector_of_courses[i];
      if (course.quarter != "null"){
        ofs << course.title << "," << course.number_of_units << "," << course.quarter << std::endl;
        vector_of_offered_courses.push_back(course);
      }
    }
  }
  ofs.close();
  for (int i = 0; i < vector_of_offered_courses.size(); ++i){
    Course course = vector_of_offered_courses[i];
